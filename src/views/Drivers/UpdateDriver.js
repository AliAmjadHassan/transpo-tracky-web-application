...
import { getSpecificEmployee } from "../../actions/EmployeesAction";
...
class UpdateEmployee extends Component {
  componentWillMount() {
    getAdminTokenfromStorage().then((response) => {
      this.props.getSpecificEmployee(this.props.match.params.id, response);
    });
  }
  ...
  render() {
    console.log("Props from View Emploee", this.props);
    return (
...
function mapStateToProps(state) {
  ...
  if (state.Employees.specificEmployee == undefined) {
    console.log("Value is undefined");
    return { Employees: state.Employees };
  } else {
    console.log(
      "Value is defined",
      state.Employees.specificEmployee.data[0].gender
    );
    return {
      //   Employees: state.Employees,
      initialValues: {
        registrationId: state.Employees.specificEmployee.data[0].id.registrationId,
        firstName: state.Employees.specificEmployee.data[0].name.firstName,
        lastName: state.Employees.specificEmployee.data[0].name.lastName,
        gender: state.Employees.specificEmployee.data[0].gender,
        contact:
          state.Employees.specificEmployee.data[0].Personal_details.contact,
        cnic: state.Employees.specificEmployee.data[0].Personal_details.CNIC,
        address:
          state.Employees.specificEmployee.data[0].Personal_details.Address,
      },
      getSpecificEmployee,
    };
  }
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators({ getSpecificEmployee }, dispatch);
}
export default reduxForm({
  ...
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(reduxForm({ asyncBlurFields: [] })(UpdateEmployee))
);
